...
public class BeautyCameraFilter extends BaseFilter {
    ...
    public void initFrameBuffer(int width, int height) {
        if (mFrameBuffer != null && (mWidth != width || mHeight != height)) {
            destroyFrameBuffer();
        }
        if (mFrameBuffer == null) {
            mWidth              = width;
        ...
        GLES30.glGenTextures(1, mFrameBufferTexture, 0);
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, mFrameBufferTexture[0]);
        // 设置滤镜方式
        ...
        GLES30.glFramebufferTexture2D(GLES30.GL_FRAMEBUFFER, GLES30.GL_COLOR_ATTACHMENT0,
                GLES30.GL_TEXTURE_2D, mFrameBufferTexture[0], 0);
        // 解绑
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, 0);
